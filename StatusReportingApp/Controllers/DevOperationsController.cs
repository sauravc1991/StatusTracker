...
{
    public class DevOperationsController : Controller
    {
        ...
        public ActionResult GetRecordsByDate(Models.DatePicker ob)
        {
            ...
            if (rid == 1)
            {
                rec = db.StatusReports.Where(x => x.UserData.GroupID==1 && x.Timestamp >= ob.Start && x.Timestamp <= ob.End).Select(x => x).ToList();
            }
            else
            {
        ...
        [HttpPost]
        public ActionResult GetRecordByTeam(Models.DatePicker ob)
        {
            int gid = Convert.ToInt32(Session["GroupID"]);
        ...
        public ActionResult Edit(StatusReport statusreport)
        {
            if (ModelState.IsValid)
            {
                ...
                if (int.Parse(Session["RoleID"].ToString()) == 2)
                    return RedirectToAction("DevUser");
                else if ((int.Parse(Session["RoleID"].ToString()) == 1))
                    return RedirectToAction("DevAdmin");
